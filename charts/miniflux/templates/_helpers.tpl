...
{{- define "miniflux.postgresql.image" -}}
...
{{- else if and (not .Values.postgresql.secret.enabled) (eq (.Values.postgresql.secret.autoCreate | toString) "false") }}
  {{- if .Values.postgresql.external.enabled -}}
    {{- printf "postgres://%s:%s@%s:%d/%s?sslmode=%s" .Values.postgresql.options.username .Values.postgresql.options.password .Values.postgresql.external.host .Values.postgresql.external.port .Values.postgresql.options.database .Values.postgresql.external.sslmode -}}
  {{- else -}}
    {{- printf "postgres://%s:%s@%s-postgresql:%d/%s?sslmode=%s" .Values.postgresql.options.username .Values.postgresql.options.password .Release.Name 5432 .Values.postgresql.options.database .Values.postgresql.options.sslmode -}}
  {{- end -}}
{{- else -}}
{{- printf "postgres://$(DB_USERNAME):$(DB_PASSWORD)@$(DB_HOST):$(DB_PORT)/$(DB_DATABASE)?sslmode=$(DB_SSL_MODE)" -}}
{{- end -}}
{{- end -}}