...
{{- else if and (not .Values.global.postgresql.secret.enabled) (eq (.Values.global.postgresql.secret.autoCreate | toString) "false") }}
  {{- if .Values.global.postgresql.external.enabled -}}
    {{- printf "postgres://%s:%s@%s:%.0f/%s" .Values.global.postgresql.options.username .Values.global.postgresql.options.password .Values.global.postgresql.external.host .Values.global.postgresql.external.port .Values.global.postgresql.options.database -}}
  {{- else -}}
    {{- printf "postgres://%s:%s@%s-postgresql:%d/%s" .Values.global.postgresql.options.username .Values.global.postgresql.options.password .Release.Name 5432 .Values.global.postgresql.options.database -}}
  {{- end -}}
{{- else -}}
